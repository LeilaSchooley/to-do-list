...
import { clickImportantStuff, showByWeek } from "./arrangeTodos";
import {
  getProjectName,
  removeTodoContainer,
  removeClassList,
...
import { editSubmitTodo, createEditForm } from "./editSubmitTodo";
    ...
    element.addEventListener("click", () => {
      Object.keys(localStorage).forEach((key) => {
        ...
        newObjArray.forEach((obj) => {
          if (
            obj.id === element.id &&
            checkForElementID(`edit-todo-${obj.id}`)
          ) {
            showTodoOptions(obj);
          }
        });
...
export { selectProject, getProjectTodos, viewToDoDetails, deleteTodo };
